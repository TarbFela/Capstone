...
mcp_status_t mcp_read_cfgn(mcp_info_t *s, uint8_t *dst, int cfg_n) {
    ...
    uint8_t cmd = (MCP_CMD_DEV_ADDR) | (MCP_CMD_ADC_REG_READ_STAT(MCP_REG_ADDR_CONFIG0 + cfg_n));

    gpio_put(s->cs,MCP_CS_SELECT);
    sleep_us(MCP_SLEEPTIME_US);

    spi_write_read_blocking(MCP_SPI, &cmd, &status, 1);
    spi_read_blocking(MCP_SPI,0x0,dst,1);
